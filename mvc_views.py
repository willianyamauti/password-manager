...
class PswGenPanel:
    def __init__(self, controller):
        ...
        self.website_entry = Entry(width=30, font=FONT, highlightthickness=0, justify=JUSTIFY)
        self.website_entry.focus()
        self.website_entry.grid(column=1, row=1, columnspan=2, sticky='W')

        # Search Button
        self.search_button = Button(text='Search', width=20,)
        self.search_button.grid(column=2, row=1, columnspan=10, sticky='W')
        self.search_button.bind("<Button>", controller.model_find_password)

        # username
        self.username_label = Label(text='Email/Username:', font=FONT)
        self.username_label.grid(column=0, row=2)
        self.username_entry = Entry(width=30, font=FONT, highlightthickness=0, justify=JUSTIFY)
        self.username_entry.insert(0, controller.model.username_temp)
        self.username_entry.grid(column=1, row=2, columnspan=2, sticky='W')
        ...
        self.password_button = Button(text='Generate Password', width=20)
        # self.password_button.place(height=24, width=120, x=500, y=246)
        self.password_button.grid(column=3, row=2, sticky='E')
        self.password_button.bind("<Button>", controller.view_generate_password)

        # hide password checkbox
        self.check_state = IntVar(value=0)
        self.hide_password_checkbox = Checkbutton(text='Hide Password', variable=self.check_state, onvalue=1, offvalue=0)
        self.check_state.get()
        # self.hide_password_checkbox.place(height=15, width=45, x=450, y=308, bordermode='outside')
        self.hide_password_checkbox.grid(column=3, row=3, sticky='W',)
        self.hide_password_checkbox.bind("<Button>", controller.view_hide_password)

        # add button
        self.add_button = Button(text='Add', width=28, font=FONT, highlightthickness=0)
        self.add_button.grid(column=1, row=4, columnspan=1,  sticky='N')
